...
INSERT INTO
    registro (valor, dtHora, fkSensor)
VALUES (0, '2024-06-01 10:00:00', 1),
...
INSERT INTO
    Fale_Conosco (nomeFac, emailFac, mensagem)
VALUES (
        'Robson Guilherme',
        'robson35@gmail.com',
        'Gostaria de um orçamento planejado.'
    ),
    (
        'João Coimbra',
        'joaocoimbra17@gmail.com',
        'Quero marcar uma reunião para aplicar o projeto.'
    ),
    (
        'Samuel',
        'samuel345@gmail.com',
        'Parabéns pelo proposta!'
    ),
    (
        'Lavinia Todarelli',
        'lavinia376@gmail.com',
        'Quero aplicar o projet urgentimente.'
    ),
    (
        'Roger Elias',
        'rogerelias243@gmail.com',
        'Tenho algumas dúvidas'
    );

SELECT s.idSensor, s.modelo, r.idRegistro, r.valor, r.dtHora, e.nomeEntrada, sh.nomeShopping
FROM
    sensor AS s
    JOIN registro AS r ON s.idSensor = r.fkSensor
    JOIN entrada AS e ON s.fkEntrada = e.idEntrada
    JOIN shopping AS sh ON e.fkShopping = sh.idShopping;

SELECT s.nomeShopping, s.numeroTel, s.cnpj, u.email, u.senha, e.cep, e.bairro, e.cidade, e.uf, e.numero
FROM
    shopping AS s
    JOIN usuario AS u ON s.idShopping = u.fkShopping
    JOIN endereco AS e ON s.idShopping = e.fkShopping;

-- view total visitantes
create view vw_total_visitantes as
select sum(valor) as total, shopping_id
from
    registro
    join sensor on fkSensor = idSensor
    join shopping on shopping_id = idShopping
group by
    shopping_id;
...
-- view pico de fluxo
create view vw_pico as
select date_format(dtHora, '%H:00') as hora, sum(valor) as total, shopping_id
from
    registro
    join sensor on fkSensor = idSensor
    join shopping on shopping_id = idShopping
...
alter view vw_media as
select shopping_id, date(dtHora) as dia, round(
        sum(valor) / count(distinct hour(dtHora)), 2
    ) as media
from
    registro
    join sensor on fkSensor = idSensor
